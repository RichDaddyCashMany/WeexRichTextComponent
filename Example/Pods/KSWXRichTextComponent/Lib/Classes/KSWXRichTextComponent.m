...
//        text = @"<p style='background-color: red;'>我是p标签<span style='display:inline-block;font-family:PingFangSC-Regular;background-color: blue; font-size:24px;padding:0px;margin-left: 10px;height: 44px;line-height:30px;text-overflow: ellipsis;white-space: nowrap;overflow: hidden;width:60px;text-align: right;border-radius: 6px;border-width: 0px;border-color: red;border-style: solid;background-image: linear-gradient(to left, blue, green, orange);'>我是span标签</span></p>";
...
- (NSAttributedString *)buildAttributeString {
    ...
    [attributedString enumerateAttribute:NSFontAttributeName inRange:NSMakeRange(0, [attributedString length]) options:0 usingBlock:^(id  _Nullable value, NSRange range, BOOL * _Nonnull stop) {
        if ([value isKindOfClass:[UIFont class]]) {
            font = value;
            CGFloat fontSize = _fontSizeForStyle > 0 ? _fontSizeForStyle : 12;
            
            if (_fontFamilyForStyle && [UIFont fontWithName:_fontFamilyForStyle size:fontSize]) {
                font = [UIFont fontWithName:_fontFamilyForStyle size:fontSize];
    ...
    [attributedString enumerateAttribute:NSParagraphStyleAttributeName inRange:NSMakeRange(0, [attributedString length]) options:0 usingBlock:^(id  _Nullable value, NSRange range, BOOL * _Nonnull stop) {
        if ([value isKindOfClass:[NSParagraphStyle class]]) {
            ...
            [attributedString addAttribute:NSBaselineOffsetAttributeName value:@((_lineHeightForStyle - font.lineHeight) / 2.0) range:range];
            
            style.minimumLineHeight = _lineHeightForStyle;
            style.maximumLineHeight = _lineHeightForStyle;
    ...
    [attributedString enumerateAttributesInRange:NSMakeRange(0, attributedString.length) options:0 usingBlock:^(NSDictionary<NSAttributedStringKey,id> * _Nonnull attrs, NSRange range, BOOL * _Nonnull stop) {
        ...
        if ([value.string hasPrefix:RICH_TAG_IDENTIFIER]) {
            ...
            NSMutableAttributedString *tagAttrString = [NSMutableAttributedString attributedStringOfImage:image lineHeight:_lineHeightForStyle descender:font.descender];
